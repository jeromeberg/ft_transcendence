...
export class AuthService {
    ...
    async loginWith42(code: string) {
        ...
        if (!tokenRes.ok) {
            if (tokenRes.status === 400) throw new BadRequestException('FORTYTWO_BAD_REQUEST');
            if (tokenRes.status === 401) throw new UnauthorizedException('FORTYTWO_UNAUTHORIZED');
            if (tokenRes.status === 403) throw new UnauthorizedException('FORTYTWO_FORBIDDEN');
            if (tokenRes.status === 404) throw new UnauthorizedException('FORTYTWO_NOT_FOUND');
        ...
        if (!meRes.ok) {
            if (meRes.status === 401) throw new UnauthorizedException('FORTYTWO_TOKEN_EXPIRED');
            if (meRes.status === 403) throw new UnauthorizedException('FORTYTWO_FORBIDDEN');
            if (meRes.status === 404) throw new UnauthorizedException('FORTYTWO_USER_NOT_FOUND');
        ...
        if (oauthAccount) {
            ...
            if (existing) username = `${me.login}_42`;

            const existingEmail = await this.usersService.findByEmail(me.email);

            if (existingEmail) throw new BadRequestException('EMAIL_ALREADY_IN_USE');
