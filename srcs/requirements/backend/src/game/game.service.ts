...
export type AssignResult =
    { status: 'joined'; room: RoomState } | { status: 'busy' } | { status: 'duplicate_session' };
...
export class GameService {
    ...
    private connectedHumanCount(room: RoomState): number {
        let n = 0;
        for (const p of room.players.values()) if (p.kind !== 'bot' && p.socketId !== null) n++;
        return n;
    }
    ...
    private userInRoom(userId: number): boolean {
        for (const room of this.rooms.values())
            for (const p of room.players.values())
                if (p.kind === 'user' && p.userId === userId && p.socketId !== null) return true;
        return false;
    }
    ...
    async assign(input: JoinInput): Promise<AssignResult> {
        ...
        else this.emitLobbyUpdate(existing);
    ...
    private startCountdown(room: RoomState): void {
        ...
        room.countdownTimer = setTimeout(() => this.onCountdownEnd(room.id), LOBBY_COUNTDOWN_MS);
        this.emitLobbyUpdate(room);
    }
    ...
    updateProgress(
        ...
        if (
            ...
            now - p.lastProgressAt < PROGRESS_MIN_INTERVAL_MS
        )
            return null;
    ...
    handleFinish(
        socketId: string,
    ): { roomId: string; allDone: boolean; position: number; playerCount: number } | null {
        const room = this.roomOf(socketId);
