...
async function main() {
    ...
    for (let i = 0; i < SEED_MATCHES; i++) {
        ...
        const endedAt = new Date(
            startedAt.getTime() + faker.number.int({ min: 30, max: 300 }) * 1000,
        );
        ...
        const results = players
            .map((user) => ({
                ...
                userId: user.id,
                wpm: parseFloat(
                    faker.number.float({ min: 30, max: 150, fractionDigits: 2 }).toFixed(2),
                ),
                position: 0,
                nbPlayers: playerCount,
                nbBots: 0,
                accuracy: parseFloat(
                    faker.number.float({ min: 75, max: 100, fractionDigits: 2 }).toFixed(2),
                ),
                finishedAt: endedAt,
            }))
    ...
    console.log(
        `Seeded: ${SEED_USERS} users, ${SEED_MATCHES} matches, ${friendshipPairs.size} friendships`,
    );
}
